...
- **Registro en `metrobus-gtfs.sinopticoplus.com` hecho (2026-08-29)**, pero resultó ser el feed **estático**, no el de tiempo real (GTFS-RT) que necesita `scripts/gtfs-rt/` — `METROBUS_GTFS_TOKEN`/`METROBUS_RT_URL` siguen sin resolverse, el parser sigue 0% verificado contra un feed real. El registro en sí requiere renovarse cada año (dato nuevo). El feed estático que sí llegó es real y mejor que lo ya cargado (`feed_info.txt` con vigencia confirmada, horarios reales por viaje) pero usa otro esquema de IDs — archivado sin integrar en `data/raw/metrobus-gtfs-estatico/` (gitignorado), decisión explícita de no tocarlo todavía mientras se prioriza `qa-rutas`. Detalle completo en `docs/handoff/01-datos.md` sección 1.3.
